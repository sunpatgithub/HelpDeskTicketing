...
{
    public class Employee_CurrentSalaryActivities : IActivities<Employee_Salary>
    {
        private readonly ApplicationDbContext adbContext;

        public Employee_CurrentSalaryActivities(ApplicationDbContext applicationDbContext)
        {
        ...
        public async Task<IEnumerable<Employee_Salary>> GetById(int id)
        {
            try
            {
                ...
                int month = DateTime.Now.Month, nextMonth;
                int year = DateTime.Now.Year, nextYear;
                if (month == 12)
                {
                    nextYear = (year + 1);
                    nextMonth = 1;
